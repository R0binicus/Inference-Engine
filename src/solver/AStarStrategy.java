	...
	public direction[] Solve(nPuzzle aPuzzle)
	{
		...
		while(Frontier.size() > 0)
		{
			...
			for(int i = 0; i < newStates.size(); i++)
			{
				...
				{
					//set parent
					newChild.Parent = thisState;
					//set cost
					newChild.Cost = thisState.Cost + 1;
					//then, work out it's heuristic value
					newChild.HeuristicValue = HeuristicValue(newStates.get(i), aPuzzle.GoalState);
					// Add cost to heuristic value
					newChild.HeuristicValue = newChild.HeuristicValue + newChild.Cost;
